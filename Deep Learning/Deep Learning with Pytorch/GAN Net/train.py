...
transforms_ = [
    ...
    transforms.Resize(int(256 * 1.12), Image.BILINEAR),
...
dataloader = DataLoader(ImageDataset(data_root, transforms_), batch_size=batch_size, shuffle=True, num_workers=8)
step = 0
...
for epoch in range(n_epoch):
    for i, batch in enumerate(dataloader):
        ...
        loss_indentity_B = loss_identity(same_B, real_B) * 5.0
        # 同样方法生成器B 生成 A
        same_A = netG_B2A(real_A)
        loss_indentity_A = loss_identity(same_A, real_A) * 5.0
        ...
        # 生成器整体的 loss
        loss_G = loss_indentity_A + loss_indentity_B + \
